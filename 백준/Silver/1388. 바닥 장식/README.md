...
### 놓친 점
1. 그래프에 직접 방문 표시 가능한 경우 그냥 그래프에 바로 표식 남겨서 방문처리 해라
2. 우선 순위 주의
   * 방문한 칸이 -인지 |인지 확인 우선 -> 이에 따라 이동 처리 달라지기 때문
   * 방문한 칸이 어떤 무늬인지 확인된 후 그래프에 바로 방문 표식 남겨서 방문 처리함
3. 더 이이서 탐색해볼 조건
   * 이동하고자 하는 좌표가 그래프에서 유효한 좌표인지
   * 이동하고자 하는 좌표의 무늬가 이전 좌표 무늬와 동일한지
